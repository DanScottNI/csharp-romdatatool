...
namespace ROMDataWin
{
    public class ApplicationConfig
    {
        ...
        public ApplicationConfig(string filename)
        {
            ...
            DefaultDataDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "ROMDataWin");
        ...
        public void SaveConfiguration()
        {
            ...
            XmlElement groupItems = xmlDoc.CreateElement("groupitemsbycategory");
            groupItems.InnerText = this.GroupByCategory.ToString();
            configNode.AppendChild(groupItems);

            XmlElement defaultData = xmlDoc.CreateElement("defaultdatadirectory");
            defaultData.InnerText = this.DefaultDataDirectory;
            configNode.AppendChild(defaultData);
        ...
        public void LoadConfiguration()
        {
            if (System.IO.File.Exists(configurationFilename) == true)
            {
                ...
                foreach (XmlNode node in xmlDoc.DocumentElement.ChildNodes)
                {
                    ...
                    if (node.Name == "groupitemsbycategory")
                    {
                        this.GroupByCategory = Convert.ToBoolean(node.InnerText);
                    }

                    if (node.Name == "defaultdatadirectory")
                    {
                        if (Directory.Exists(node.InnerText))
                        {
                            this.DefaultDataDirectory = node.InnerText;
                        }
                    }
                }
            }
